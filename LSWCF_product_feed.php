    ...
    foreach ( $products as $product ) {
        ...
        if ( $product_obj->is_type( 'variable' ) ) {
			...
			break;
                ...
                $GPID = wp_strip_all_tags($product->get_attribute( 'google-product-id' ));
                $output .= "\t\t\t" . '<g:google_product_category>' . strlen($GPID) == 0 ? 232 : $GPID . '</g:google_product_category>' . PHP_EOL;
                $output .= "\t\t" . '</item>' . PHP_EOL;
            }
        } 
    }
...
add_filter( 'plugin_action_links_rss-for-woo-dev/LSWCF_product_feed.php', 'LSWCF_setup_view_feed_link' );
